...
class XMLManager:
    def __init__(self, config):
        ...
        self.body_components =['base_link', 'FL_hip_link', 'FR_hip_link', 'RL_hip_link', 'RR_hip_link', 'FL_shoulder_link',
                                'FR_shoulder_link', 'RL_shoulder_link', 'RR_shoulder_link', 'FL_leg_link', 'FR_leg_link',
                                  'RL_leg_link', 'RR_leg_link', 'FL_wheel_link', 'FR_wheel_link', 'RL_wheel_link', 'RR_wheel_link']

        self.precision_attr_map = config["random_table"]["precision"]

    def get_model_path(self):
        original_model_path = os.path.join(self.cur_dir, '..', 'assets', 'xml', 'wheeldog_p_v2.xml')
        ...
        for body in root.findall('.//body'):
            if body.attrib.get('name') in ['FL_wheel_link', 'FR_wheel_link', 'RL_wheel_link', 'RR_wheel_link']:
        ...
        if bool(hm_cfg.get("enabled", False)):
            ...
            for i in range(res_y):
                for j in range(res_x):
                    site_name = f"dataset_heightmap_site_{i}_{j}"
                    site_element = ET.Element('site', {
                        'name': site_name,
                        'type': 'sphere',
                        'size': '0.00000001',
                        'pos': '0 0 -1',
                        'rgba': '1 0.5 0 0.0000001',
                        'group': '0',
                    })
        ...
        randomized_model_path = os.path.join(self.cur_dir, '..', 'assets', 'xml', 'applied_wheeldog_p_v2.xml')
